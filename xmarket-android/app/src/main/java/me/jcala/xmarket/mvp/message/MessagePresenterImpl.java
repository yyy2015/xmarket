    ...
    private void initList(List<Message> messageList){
        adapter=new RecyclerCommonAdapter<Message>(context,
            ...
            public void convert(RecyclerViewHolder viewHolder, Message item) {
                if (item.getKind()==0){
                    viewHolder.setLineBgColor(R.id.message_kind_bg,context.getResources().getColor(R.color.new_pink_1));
                    ...
                    viewHolder.setLineBgColor(R.id.message_kind_bg,context.getResources().getColor(R.color.new_yellow_1));
                    viewHolder.setText(R.id.message_kind_title,"收到捐赠商品，点击给捐赠者发送短信");
                    viewHolder.setSendMsgListener(R.id.message_item,item.getUserPhone(),item.getUsername());
                } else if (item.getKind() ==1){
                    viewHolder.setLineBgColor(R.id.message_kind_bg,context.getResources().getColor(R.color.new_green_1));
                    viewHolder.setText(R.id.message_kind_title,"收到购买请求，点击确认交易");
